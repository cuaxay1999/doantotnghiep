<template lang="html">
  <div>
    <v-header title-text="Chỉnh sửa người dùng" title-icon="user" />
      ...
      <main class="bg-white rounded-lg p-[1em] py-[2em] max-w-[1140px] mx-auto box-shadow-bordered">
        <el-form ref="form" label-position="top" :model="form" :rules="rules">
          ...
          <div class="text-right mt-1-em">
            <el-button class="btn--green btn" icon="el-icon-circle-check" :loading="loading" @click="updateUser">
              {{ $t('button.save') }}
            </el-button>
          </div>
        </el-form>
...
export default {
  ...
  async created() {
    await Promise.all([this.getDevices(), this.getUser()])
  },
  methods: {
    async updateUser() {
      try {
        ...
        const { name, phone, email, device_id } = this.form
    ...
    async getUser() {
      try {
        ...
        this.form = {
          ...res.data.item,
          device_id: Number(res.data.item.device_id)
        }
      } catch (e) {
        console.log(e)
      } finally {
